...
async def snapshot_updater_loop():
    """Background task: update snapshot tiap 60 detik agar market_bot punya data fresh."""
    logging.info("Snapshot updater loop started")
    while True:
        try:
            await asyncio.get_event_loop().run_in_executor(None, update_market_snapshot)
            logging.info("market_bot: snapshot updated")
        except Exception as e:
            logging.error(f"market_bot snapshot updater error: {e}")
